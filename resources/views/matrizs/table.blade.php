<table class="table table-responsive table-striped table-bordered table-hover table-checkable datatable" id="matrizs-table">
    ...
    @foreach($matrizs as $matriz)
        <tr>
            <td>{!! $matriz->nombre !!}</td>
            <td><a href="{{route('modelos.show',$matriz->modelo_id)}}"><u>{!! $matriz->modelo->nombre !!}</u></a></td>
            <td>{!! $matriz->periodo->nombre !!}</td>
            <td>{!! $matriz->carrera_id!=null?$matriz->carrera->nombre:'-' !!}</td>
            <td>{!! $matriz->tipoMatriz->nombre !!}</td>
